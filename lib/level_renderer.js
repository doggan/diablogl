...
let PILLAR_BLOCK_OFFSETS = new Array(16);
// Left side.
PILLAR_BLOCK_OFFSETS[0] = {
    x: 0,
    y: BLOCK_HEIGHT * 7
};
PILLAR_BLOCK_OFFSETS[2] = {
    x: 0,
    y: BLOCK_HEIGHT * 6
};
PILLAR_BLOCK_OFFSETS[4] = {
    x: 0,
    y: BLOCK_HEIGHT * 5
};
PILLAR_BLOCK_OFFSETS[6] = {
    x: 0,
    y: BLOCK_HEIGHT * 4
};
PILLAR_BLOCK_OFFSETS[8] = {
    x: 0,
    y: BLOCK_HEIGHT * 3
};
PILLAR_BLOCK_OFFSETS[10] = {
    x: 0,
    y: BLOCK_HEIGHT * 2
};
PILLAR_BLOCK_OFFSETS[12] = {
    x: 0,
    y: BLOCK_HEIGHT * 1
};
PILLAR_BLOCK_OFFSETS[14] = {
    x: 0,
    y: BLOCK_HEIGHT * 0
};
// Right side.
PILLAR_BLOCK_OFFSETS[1] = {
    x: BLOCK_WIDTH,
    y: BLOCK_HEIGHT * 7
};
PILLAR_BLOCK_OFFSETS[3] = {
    x: BLOCK_WIDTH,
    y: BLOCK_HEIGHT * 6
};
PILLAR_BLOCK_OFFSETS[5] = {
    x: BLOCK_WIDTH,
    y: BLOCK_HEIGHT * 5
};
PILLAR_BLOCK_OFFSETS[7] = {
    x: BLOCK_WIDTH,
    y: BLOCK_HEIGHT * 4
};
PILLAR_BLOCK_OFFSETS[9] = {
    x: BLOCK_WIDTH,
    y: BLOCK_HEIGHT * 3
};
PILLAR_BLOCK_OFFSETS[11] = {
    x: BLOCK_WIDTH,
    y: BLOCK_HEIGHT * 2
};
PILLAR_BLOCK_OFFSETS[13] = {
    x: BLOCK_WIDTH,
    y: BLOCK_HEIGHT * 1
};
PILLAR_BLOCK_OFFSETS[15] = {
    x: BLOCK_WIDTH,
    y: BLOCK_HEIGHT * 0
};
...
function drawPillarSide(pillarBlocks, blockStartIndex, xOffset, yOffset, zCoord) {
    ...
    for (let i = blockStartIndex; i >= 0; i -= 2) {
        ...
        if (block) {
            ...
            isFirst = false;
            let rect = PILLAR_BLOCK_OFFSETS[i];
            let extraYOffset = moveUp ? 1 : 0;

            let tileInfo = tilesetInfos[block.frameNum];
            let geometry = textureIdToGeometryLUT[tileInfo.textureId];
            ...
            if (!geometry) {
                ...
                let material = MaterialMgr.getBasicMaterial(tileInfo.textureId);
                materials.push(material);

                textureIdToGeometryLUT[tileInfo.textureId] = geometry;
            }

            let vertOffset = geometry.vertices.length;
...
function _drawDungeon(dunFile, tilFile, minFile, scene) {
    ...
    let SpriteMgr = require('./core/sprite_mgr');
    ...
    for (let row = 0; row < dunFile.rowCount; row++) {
        for (let col = 0; col < dunFile.colCount; col++) {
            ...
            if (pillarIndex !== null) {
                let offset = getPillarOffset(col, row);
