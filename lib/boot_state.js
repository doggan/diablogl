/**
 * TODO:
 * - look into getting blizzconv running and export all pngs from MPQ file
 * - allow viewing of actual dungeon pieces so we can figure out format / rendering order
 * 	 for arch overlays, underpasses, and transparent walls.
 */
...
class BootState {
    preload() {
        ...
        this.game.load.image('test_pillar', 'assets/test_pillar.png');
    ...
    create() {
        ...
        mapUtils.createMap(this.game, 'maps/map_0', (err, map) => {
            ...
            function renderLayer(map, layerName, parentGroup, omg) {
                ...
                for (let y = 0; y < map_h; y++) {
                    for (let x = 0; x < map_w; x++) {
                        ...
                        // let a = parentGroup.create(xPos, yPos, atlasName, tileInfo.tileId);
                        
                        if (omg === true) {
                            let a = parentGroup.create(xPos, yPos, 'test_pillar');
                            a.zz = y * map_w + x;
                            console.log('setting to: ' + a.zz);
                        }
                         else {
                             let a = parentGroup.create(xPos, yPos, atlasName, tileInfo.tileId);
                         }
            ...
            renderLayer(map, 'background', this.backgroundGroup, false);
            renderLayer(map, 'object2', this.objectGroup, true);
    ...
    update() {
        ...
        // TODO:
        // - for all objects in the object group
        //  - sort based on:
        //      - smaller x gets rendered first (back)
        //          - if x is equal, smaller y gets rendered first (back)
        //  
        //  
        // - for all sprites in object group
        //   - z == tile.y * width + tile.x

        // console.log(this.objectGroup.children);
        
        if (this.aaa) {
        }else {
            // console.log(this.objectGroup.children[0]);
            this.aaa = true;
        ...
        this.objectGroup.sort('zz', Phaser.Group.SORT_ASCENDING);
