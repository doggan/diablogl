...
class Player {
    ...
    get position() {
        return [this._sprite.position.x, this._sprite.position.y];
    }
    ...
    _setTargetTilePos(targetTilePos) {
        // Move speed.
        this._currentMoveSpeed = 60;
        // Update current grid position.
        this._currentTilePos = targetTilePos;

        // Target position.
        this._currentTargetPos = LevelController.map.tileToWorldPosCenter(targetTilePos);
    ...
    _followPath(dt) {
        let moveAmount = this._currentMoveSpeed * dt;
        ...
        if (this._currentDistanceRemainingToTargetPos <= 0.0) {
            ...
            this.position = [
                this.position[0] + this._currentMoveDirection[0] * moveAmount,
                this.position[1] + this._currentMoveDirection[1] * moveAmount
            ];
        }
    }
