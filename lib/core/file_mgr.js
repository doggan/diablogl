...
function createFileLoader(loadUnit, path) {
    return loadFileImpl(path, function(buffer) {
        let file = createFileImpl(path, buffer);
...
function _loadFiles(loadUnit, paths, onFinishedCallback) {
    let fileLoaderWork = new Array(paths.length);
...
function _unloadFiles(loadUnit) {
    ...
    let unloadPaths = [];
    for (let key in loadedFileInfos) {
        let info = loadedFileInfos[key];
    ...
    // Perform deletion.
    for (let i = 0; i < unloadPaths.length; i++) {
        let path = unloadPaths[i];
